...
async def lifespan(app: FastAPI):
    ...
    try:
        result = subprocess.run(
            ["bash", "scripts/check_ntp_drift.sh"], capture_output=True, text=True, timeout=5
        )
    ...
    # Initialize managers
    app_state.instrument_manager = InstrumentManager(app_state.kite, app_config.universe, settings)

    app_state.risk_manager = RiskManager(app_config.risk)
    app_state.exit_manager = ExitManager(app_config.exits)

    app_state.execution_engine = ExecutionEngine(app_state.kite, app_config.execution, settings)

    # Initialize market data stream
    app_state.market_data_stream = MarketDataStream(settings=settings, window_seconds=[1, 5])
    ...
    async def refresh_prelive_metrics_task():
        ...
        def _latest_day2_json():
            files = sorted(
                DAY2_DIR.glob("day2_*.json"), key=lambda p: p.stat().st_mtime, reverse=True
            )
...
def compliance_status():
    ...
    fam_ok, fam_msg = (
        (True, "provider mode")
        if mode_profile != "PERSONAL"
        else compliance.check_family_only(active_clients, wl)
    )
...
async def change_mode(request: ModeChangeRequest):
    ...
    if request.mode == "LIVE":
        if request.confirmation != "CONFIRM LIVE TRADING":
            raise HTTPException(
                status_code=403, detail="LIVE mode requires confirmation: 'CONFIRM LIVE TRADING'"
            )
        ...
        def _validate_day2_json_or_403():
            ...
            if age > FRESH_SECS:
                raise HTTPException(
                    status_code=403, detail=f"Day-2 JSON stale (age={int(age / 3600)}h, max 36h)"
                )
            ...
            if "heartbeats" not in data or not isinstance(data["heartbeats"], dict):
                raise HTTPException(
                    status_code=403, detail="Day-2 JSON: heartbeats missing or invalid"
                )
            ...
            if data.get("status") != "PASS":
                raise HTTPException(
                    status_code=403,
                    detail=f"Day-2 JSON status={data.get('status')} (expected PASS)",
                )

            if int(data.get("leader", 0)) != 1:
                raise HTTPException(
                    status_code=403, detail=f"Day-2 JSON leader={data.get('leader')} (expected 1)"
                )

            if any(float(hb.get(k, 999)) >= 5 for k in hb_keys):
                raise HTTPException(
                    status_code=403,
                    detail=f"Day-2 JSON: heartbeats >= 5s (market={hb.get('market')}, orders={hb.get('orders')}, scan={hb.get('scan')})",
                )

            if int(data.get("leader_changes", 999)) > 2:
                raise HTTPException(
                    status_code=403,
                    detail=f"Day-2 JSON: leader_changes={data.get('leader_changes')} (>2)",
                )

            if int(data.get("duplicates", 999)) != 0:
                raise HTTPException(
                    status_code=403, detail=f"Day-2 JSON: duplicates={data.get('duplicates')} (!=0)"
                )

            if int(data.get("orphans", 999)) != 0:
                raise HTTPException(
                    status_code=403, detail=f"Day-2 JSON: orphans={data.get('orphans')} (!=0)"
                )

            if float(data.get("flatten_ms", 9999)) > 2000:
                raise HTTPException(
                    status_code=403,
                    detail=f"Day-2 JSON: flatten_ms={data.get('flatten_ms')}ms (>2000ms)",
                )
        ...
        if day2_json and day2_json.exists():
            try:
                ...
                logger.info(
                    "Day-2 scorer JSON check passed with full validation", json_file=str(day2_json)
                )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error validating Day-2 scorer JSON: {str(e)}"
                )
        else:
        ...
        if settings.app_mode.value == "LIVE":
            raise HTTPException(
                status_code=400, detail="Already in LIVE mode. No need to switch again."
            )
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                logger.info(
                    "Mode change logged to AuditLog",
                    config_sha=runtime_config_sha,
        ...
        if override_reason:
            ...
            try:
                ...
                snapshot_file = (
                    snapshot_dir
                    / f"override_snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                )
                with open(snapshot_file, "w") as f:
                    ...
                    f.write(
                        f"Git HEAD: {subprocess.run(['git', 'rev-parse', 'HEAD'], capture_output=True, text=True).stdout.strip()}\n"
                    )
                    f.write(
                        f"Config SHA: {runtime_config_sha if 'runtime_config_sha' in locals() else 'unknown'}\n"
                    )
    ...
    return {"status": "success", "mode": request.mode, "timestamp": datetime.now().isoformat()}
...
async def get_positions():
    ...
    positions_list = (
        app_state.orchestrator.positions if app_state.orchestrator else app_state.positions
    )
...
async def close_position(position_id: str):
    ...
    try:
        order = await app_state.execution_engine.close_position(position, reason="MANUAL_CLOSE")

        if order:
            position.status = PositionStatus.CLOSED

            return {"status": "success", "position_id": position_id, "order_id": order.order_id}
        else:
...
async def run_backtest(request: BacktestRequest):
    ...
    try:
        ...
        if not strategies:
            raise HTTPException(
                status_code=400, detail="No strategies configured. Check configs/app.yaml"
            )
