"""Bollinger Bands Strategy for Indian Markets (Nifty)

Based on top 10 Nifty trading strategies.
Bollinger Bands mean reversion strategy.

Strategy Logic:
- Buy when price touches or moves below lower band (oversold)
- Sell when price touches or moves above upper band (overbought)
...
class BollingerBandsStrategy(Strategy):
    ...
    2. Buy when price touches lower band (oversold)
    3. Sell when price touches upper band (overbought)
    4. Use ATR for stop-loss and targets
    5. Mean reversion targets
    ...
    def generate_signals(self, context: StrategyContext) -> List[Signal]:
        ...
        min_bars = max(self.bb_period, self.atr_period) + 10
        ...
        # Get Bollinger Bands from latest bar
        bb_upper = latest_bar.bb_upper
        bb_middle = latest_bar.bb_middle
        bb_lower = latest_bar.bb_lower
        
        if bb_upper is None or bb_middle is None or bb_lower is None:
            logger.debug("Bollinger Bands not available", token=token)
            return []
        ...
        # Check for oversold (price touches lower band)
        if current_price <= bb_lower:
            ...
            if signal:
                ...
                logger.info(
                    "Bollinger Bands LONG signal (oversold)",
                    instrument=context.instrument.tradingsymbol,
                    entry=signal.entry_price,
                    bb_lower=bb_lower
                )
        
        # Check for overbought (price touches upper band)
            ...
            if signal:
                ...
                self.last_signal_time[token] = context.timestamp
                logger.info(
                    "Bollinger Bands SHORT signal (overbought)",
                    instrument=context.instrument.tradingsymbol,
                    entry=signal.entry_price,
                    bb_upper=bb_upper
                )
    ...
    def _create_long_signal(
        ...
        stop_loss = bb_lower - (atr * self.atr_stop_mult)
        ...
        # Target at middle band (mean reversion)
        ...
        signal = Signal(
            ...
            confidence=0.70,
            rationale=f"Bollinger Bands LONG: Price {entry_price:.2f} at lower band {bb_lower:.2f}",
            features={
                "bb_upper": bb_upper,
                "bb_middle": bb_middle,
                "bb_lower": bb_lower,
    ...
    def _create_short_signal(
        ...
        stop_loss = bb_upper + (atr * self.atr_stop_mult)
        ...
        signal = Signal(
            ...
            take_profit_2=take_profit_2,
            confidence=0.70,
            rationale=f"Bollinger Bands SHORT: Price {entry_price:.2f} at upper band {bb_upper:.2f}",
            features={
                "bb_upper": bb_upper,
                "bb_middle": bb_middle,
                "bb_lower": bb_lower,
    ...
    def validate(self, context: StrategyContext) -> bool:
        """Validate Bollinger Bands strategy can run"""
